 ...
 *
 * This used to be its own top-level tab, which asked people to understand
 * that "Catalog" and "Market" were two windows onto one set of shoes. They
 * aren't — a pair on the grid and the same pair on the order book are the
 * same row rendered two ways. So it now lives inside the Market as a view
 * switch: Chart, Book, Grid, Customs.
 *
 * Every link it emits stays inside /market so the tab never changes
 * underneath you mid-browse.
 */
...
export default async function CatalogBoard({
  ...
  const gClean = gRaw.trim().toLowerCase();
  const explicit: Mode | null = gClean === "foryou" ? "foryou" : asLane(gClean);
  ...
  return (
    <div className="mt-6">
        ...
        <div className="mt-4 flex max-w-xl flex-wrap items-center justify-between gap-3 rounded-xl border border-volt/40 bg-volt/10 px-4 py-3">
          <p className="text-sm text-white">
            ...
            <span className="text-smoke">
              Sign in and tell us who you shop for — we&apos;ll deal your lane first, every visit.
            </span>
          </p>
      ...
      {/* Search + brand rail */}
      <form action="/market" className="mt-5 flex max-w-xl gap-2">
        <input type="hidden" name="board" value="catalog" />
        <input
          name="q"
          defaultValue={q}
          maxLength={80}
